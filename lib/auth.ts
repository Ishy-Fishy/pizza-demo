...
export async function verifyAuth(req: NextRequest) {
    const token = req.cookies.get(AuthConstants.UserToken)?.value
...
export async function setUserCookie(res: NextResponse) {
    ...
    res.cookies.set(AuthConstants.UserToken, token, {
        httpOnly: true,
...
export function expireUserCookie(res: NextResponse) {
    res.cookies.set(AuthConstants.UserToken, '', { httpOnly: true, maxAge: 0 })
    return res
}
